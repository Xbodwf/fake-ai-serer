...
import type { PendingRequest, Model, WSMessage, Stats, SystemSettings } from '../types';
...
export function ServerProvider({ children }: { children: ReactNode }) {
  ...
  const [settings, setSettings] = useState<SystemSettings>({ streamDelay: 500 });
  ...
  const handleMessage = useCallback((msg: WSMessage) => {
    switch (msg.type) {
      case 'request': {
        const payload = msg.payload as { requestId: string; data: any };
  ...
  const addModel = useCallback(async (model: Partial<Model>) => {
  ...
  const updateModel = useCallback(async (id: string, model: Partial<Model>) => {
